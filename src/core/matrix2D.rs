...
use crate::substitution::{back_substitution, forward_substitution};
use std::{
    any::type_name,
    fmt::{self, Display},
...
impl<T> Matrix2D<T> {
    ...
    pub fn inverse(&self) -> Result<Matrix2D<f64>, Matrix2DError>
    where
        Matrix2D<f64>: for<'a> TryFrom<&'a Matrix2D<T>>,
    {
        if self.height != self.width {
            return Err(Matrix2DError::NonSquareMatrix);
        }
        let coeff_matrix: &Matrix2D<f64> =
            &self
                .try_into()
                .map_err(|_| Matrix2DError::ConversionFailed {
                    from: type_name::<T>(),
                    to: type_name::<f64>(),
                })?;
        ...
        let (l, u, p) =
            lu_pivot_decomposition(coeff_matrix).map_err(|_| Matrix2DError::SingularMatrix)?;

        let mut inverse_matrix = Matrix2D::full(0.0, size, size);
    ...
    pub fn reshape(&mut self, height: usize) -> Result<(), Matrix2DError> {
        let size = self.height * self.width;
        if !size.is_multiple_of(height) {
            return Err(Matrix2DError::InvalidReshape {
                size,
                new_height: height,
            });
        }
    ...
    pub fn rows(&self) -> Matrix2DRows<'_, T> {
        Matrix2DRows {
            data: &self.inner,
            width: self.width,
    ...
    pub fn rows_mut(&mut self) -> Matrix2DRowsMut<'_, T> {
        Matrix2DRowsMut {
    ...
    pub fn from_flat<D>(
        ...
        height: usize,
        width: usize,
    ) -> Result<Self, Matrix2DError>
    where
        D: AsRef<[T]>,
        T: Clone,
    {
        let vec_len = inner.as_ref().len();
        let Matrix2D_size = height * width;
        if vec_len > Matrix2D_size || Matrix2D_size == 0 {
            return Err(Matrix2DError::InvalidShape {
                input_size: (vec_len),
                output_size: (Matrix2D_size),
            });
        }

        let inner = inner.as_ref().to_vec();
        if vec_len < Matrix2D_size {
            let mut new_inner = inner.clone();
            new_inner.resize(Matrix2D_size, default_val);
            return Ok(Self {
                inner: new_inner,
                height,
                width,
    ...
    pub fn dot(&self, rhs: &Self) -> Result<Self, Matrix2DError>
    where
        T: Copy + std::default::Default + std::ops::AddAssign + std::ops::Mul<Output = T>,
    {
        ...
        if self.width != rhs.height {
            return Err(Matrix2DError::InvalidDotShape {
                lhs: self.width,
                rhs: rhs.height,
            });
        }
        let mut result = Matrix2D::full(T::default(), self.height, rhs.width);
...
// Mul implementation for &Matrix2D * &Matrix2D (&matrix * &matrix)
impl<'b, T> Mul<&'b Matrix2D<T>> for &Matrix2D<T>
where
    T: Mul<Output = T> + Clone + std::default::Default + std::marker::Copy + std::ops::AddAssign,
{
    type Output = Matrix2D<T>;

    fn mul(self, rhs: &'b Matrix2D<T>) -> Matrix2D<T> {
...
// Mul implementation for Matrix2D * scalar (matrix * scalar)
    ...
    T: Mul<Output = T> + Clone + std::default::Default + std::marker::Copy,
{
    ...
    fn mul(self, rhs: T) -> Matrix2D<T> {
        let mut result = Matrix2D::full(T::default(), self.height, self.width);
        for i in 0..self.height {
...
// Div implementation for &Matrix2D / scalar (&matrix / scalar)
...
impl<T: Copy> Matrix2D<T> {
    ...
    /// Create Matrix2D with all elements being the specified value
    ...
    {
        let mut ident_mat: Matrix2D<T> = Matrix2D::full(T::from(0), size, size);
        for i in 0..size {
...
/// Iterator for Matrix2D
pub struct Matrix2DRows<'a, T> {
    data: &'a [T],
    width: usize,
    /// remaining row count
    remaining: usize,
...
impl<'a, T> Iterator for Matrix2DRows<'a, T> {
    type Item = &'a [T];
...
/// Mut iterator for Matrix2D
pub struct Matrix2DRowsMut<'a, T> {
    data: &'a mut [T],
    width: usize,
    remaining: usize,
}

impl<'a, T> Iterator for Matrix2DRowsMut<'a, T> {
    type Item = &'a mut [T];
...
// Convert a nested Vec to Matrix2D
impl<T> TryFrom<Vec<Vec<T>>> for Matrix2D<T> {
    type Error = Matrix2DError;
...
// &Vec<Vec<T>> -> Matrix2D<U>
impl<T: Clone, U: TryFrom<T>> TryFrom<&Vec<Vec<T>>> for Matrix2D<U> {
    type Error = Matrix2DError;
    ...
    fn try_from(values: &Vec<Vec<T>>) -> Result<Self, Self::Error> {
        ...
        for row in values {
            ...
            for x in row {
                inner.push(U::try_from(x.clone()).map_err(|_| {
                    Matrix2DError::ConversionFailed {
                        from: type_name::<T>(),
                        to: type_name::<U>(),
                    }
                })?);
...
// &Matrix2D<T> -> Matrix2D<U>
impl<T: Clone, U: TryFrom<T>> TryFrom<&Matrix2D<T>> for Matrix2D<U> {
    type Error = Matrix2DError;

    fn try_from(arr: &Matrix2D<T>) -> Result<Self, Self::Error> {
        if arr.is_empty() {
            return Ok(Self {
                inner: vec![],
                height: 0,
        ...
        for x in &arr.inner {
            inner.push(
                U::try_from(x.clone()).map_err(|_| Matrix2DError::ConversionFailed {
                    from: type_name::<T>(),
                    to: type_name::<U>(),
                })?,
            );
...
impl<'a, T> IntoIterator for &'a Matrix2D<T> {
    type Item = &'a [T];
    type IntoIter = Matrix2DRows<'a, T>;
...
impl<'a, T> IntoIterator for &'a mut Matrix2D<T> {
    type Item = &'a mut [T];
    type IntoIter = Matrix2DRowsMut<'a, T>;
...
// Convert a nested array like `&[[1, 2], [3, 4]]` to Matrix2D
impl<T, const M: usize, const N: usize> From<&[[T; N]; M]> for Matrix2D<T>
where
    T: Clone,
{
...
// Allow indexing Matrix2D items like `arr[(0, 1)]`
impl<T> Index<(usize, usize)> for Matrix2D<T> {
    ...
    fn index(&self, idx: (usize, usize)) -> &Self::Output {
        ...
        if row >= self.height || col >= self.width {
            panic!(
                "Out of bound index ({row},{col}) into Matrix2D of shape ({},{})",
                self.height, self.width
            )
        }
...
// Allow indexing Matrix2D rows like `arr[1]`
impl<T> Index<usize> for Matrix2D<T> {
    ...
    fn index(&self, row: usize) -> &Self::Output {
        if row >= self.height {
            panic!(
                "Out of bound row index {row} into Matrix2D of shape ({},{})",
                self.height, self.width
            )
        }
...
impl Rounding for Matrix2D<f64> {
    fn round_to_decimal(&self, decimals: u32) -> Matrix2D<f64> {
...
impl<T: PartialEq> PartialEq<Vec<Vec<T>>> for Matrix2D<T> {
...
impl<T: PartialEq> PartialEq<Matrix2D<T>> for Vec<Vec<T>> {
...
mod tests {
    ...
    fn test_getting_shape() {
        let data = Matrix2D::from(&[[1, 2, 3], [6, 5, 4]]);
        assert_eq!(data.shape(), (2, 3));

        let data: Matrix2D<i32> = Matrix2D::from(&[[1, 2]; 0]);
        assert_eq!(data.shape(), (0, 2));

        let data: Matrix2D<i32> = Matrix2D::from(&[[]; 0]);
        assert_eq!(data.shape(), (0, 0));

        let data: Matrix2D<i32> = Matrix2D::from(&[[], []]);
    ...
    fn test_mut_indexing_item() {
        let mut data = Matrix2D::from(&[[1, 2, 3], [6, 5, 4]]);
        data[(1, 2)] = 10;
        data[(0, 0)] = 11;
    ...
    fn test_mut_index_out_of_bounds_panics() {
        let mut data = Matrix2D::from(&[[1, 2, 3], [4, 5, 6]]);
    ...
    fn test_rows_mut_iterator_allows_mutation() {
        ...
        let expected = Matrix2D::from(&[[3, 2, 1], [6, 5, 4]]);
    ...
    fn test_reshape() {
        ...
        let expected = Matrix2D::from(&[[1, 2], [3, 6], [5, 4]]);
    ...
    fn test_reshape_invalid_height_errors() {
        let mut data = Matrix2D::from(&[[1, 2], [3, 4]]);
        let err = data
            .reshape(3)
    ...
    fn test_transpose() {
        let mut data = Matrix2D::from(&[[1, 2, 3], [6, 5, 4]]);
        data.transpose_mut();
        let expected = Matrix2D::from(&[[1, 6], [2, 5], [3, 4]]);
        assert_eq!(data, expected);
    }

    #[test]
    fn test_arr_from_vec() {
        let data = Matrix2D::try_from(vec![vec![1, 2, 3], vec![6, 5, 4]]).unwrap();
        let expected = Matrix2D::from(&[[1, 2, 3], [6, 5, 4]]);
        assert_eq!(data, expected);
    }
    ...
    fn test_try_from_inconsistent_rows_returns_error() {
        let err = Matrix2D::try_from(vec![vec![1, 2, 3], vec![4, 5]])
            .expect_err("rows with different widths should error");

        assert!(matches!(err, Matrix2DError::InconsistentRowLengths));
    }

    #[test]
    fn test_try_from_empty_vec_creates_empty_arr() {
        let data = Matrix2D::try_from(Vec::<Vec<i32>>::new()).unwrap();
    ...
    fn test_map_transforms_elements() {
        let data = Matrix2D::from(&[[1, 2], [3, 4]]);
        let mapped = data.map(|value| value * 2);
        let expected = Matrix2D::from(&[[2, 4], [6, 8]]);
    ...
    // --- from flat vector to Matrix2D ---

    #[test]
    fn test_from_flat() {
        let data = Matrix2D::from_flat(vec![1, 2, 3, 4, 5, 6], 0, 2, 3).unwrap();
        let out = Matrix2D::from(&[[1, 2, 3], [4, 5, 6]]);
    ...
    fn test_from_flat_slice() {
        let data = Matrix2D::from_flat(&[1, 2, 3, 4, 5, 6], 0, 2, 3).unwrap();
        let out = Matrix2D::from(&[[1, 2, 3], [4, 5, 6]]);
    ...
    fn test_from_flat_with_default() {
        let data = Matrix2D::from_flat(vec![1, 2, 3, 4], 0, 2, 3).unwrap();
        let out = Matrix2D::from(&[[1, 2, 3], [4, 0, 0]]);
    ...
    fn test_from_flat_full_zeros() {
        let flat_data = Matrix2D::from_flat(&vec![], 0, 2, 3).unwrap();
        let full_data = Matrix2D::full(0, 2, 3);
    ...
    fn test_from_flat_slice_full_zeros() {
        let data = Matrix2D::from_flat(&[], 0, 2, 3).unwrap();
        let out = Matrix2D::from(&[[0, 0, 0], [0, 0, 0]]);
    ...
    #[test]
    fn test_mat_mul_mat() {
        let arr1 = Matrix2D::from_flat(vec![1, 2, 3, 4, 5, 6], 0, 2, 3).unwrap();
        let arr2 = Matrix2D::from_flat(vec![7, 8, 9, 10, 11, 12], 0, 3, 2).unwrap();

        let expected = Matrix2D::from_flat(vec![58, 64, 139, 154], 0, 2, 2).unwrap();
    ...
    #[test]
    fn test_vec_mul_mat() {
        let arr1 = Matrix2D::from_flat(vec![3, 4, 2], 0, 1, 3).unwrap();
        let arr2 =
            Matrix2D::from_flat(vec![13, 9, 7, 15, 8, 7, 4, 6, 6, 4, 0, 3], 0, 3, 4).unwrap();
    ...
    fn test_scalar_mul_mat() {
        let scal = Matrix2D::from_flat(vec![2], 0, 1, 1).unwrap();
        let mat = Matrix2D::from_flat(vec![4, 0, 1, -9], 0, 2, 2).unwrap();
    ...
    fn test_Div_trait_mat_div_scalar() {
        let mat = Matrix2D::from_flat(vec![1.778, 0.0, 1.778], 0.0, 3, 1).unwrap();
        let result = mat / 1.778;
        let expected = Matrix2D::from(&[[1.0], [0.0], [1.0]]);
    ...
    #[test]
    fn test_display() {
        let data = Matrix2D::from(&[[1.2, 34.5678], [789.02, 0.123]]);
        let out = format!("{data}");
        let expected = r#"
[[    1.2, 34.5678 ]
 [ 789.02,   0.123 ]]"#;
    ...
    #[test]
    fn test_int_max() {
        let data = Matrix2D::from(&[[12], [10], [11], [20], [9]]);
    ...
    #[test]
    fn test_float_max() {
        let data = Matrix2D::from(&[[23.3], [12.4], [23.4], [10.4]]);
        let result = data.max().unwrap();
        let expected = 23.4;
    ...
    fn test_known_inverse() {
        let matrix = Matrix2D::from(&[[3, 0, 2], [2, 0, -2], [0, 1, 1]]);
        let result = matrix.inverse().unwrap();
        let rounded_result = result.round_to_decimal(1);
        let expected = Matrix2D::from(&[[0.2, 0.2, 0.0], [-0.2, 0.3, 1.0], [0.2, -0.3, 0.0]]);
